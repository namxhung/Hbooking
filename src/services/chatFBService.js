    ...
    switch (name) {
        case "intent":
            if (entity.value === 'Doctors') {
                await callSendAPI(sender_psid, "Bạn đang tìm kiếm thông tin về Doctors, xem thêm ở link bên dưới nhé.");
                let title = "Doctors Care";
                let subtitle = 'Thông tin Doctors làm việc tại Doctors Care';
                await callSendAPIv2(sender_psid, title, subtitle, DOCTOR_IMAGE_URL, DOCTOR_URL);
                await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
            }
            if (entity.value === 'tiêu hóa') {
                await callSendAPI(sender_psid, "Bạn đang gặp vấn đề về bệnh đường tiêu hóa, xem thêm danh sách Doctors chuyên khoa TIÊU HÓA.");
                let title = "Chuyên khoa khám bệnh";
                let subtitle = 'Thông tin Doctors chuyên khoa tiêu hóa';
                await callSendAPIv2(sender_psid, title, subtitle, TIEUHOA_IMAGE_URL, TIEUHOA_URL);
                await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
            }
            if (entity.value === 'cơ-xương-khớp') {
                await callSendAPI(sender_psid, "Bạn đang gặp vấn đề về cơ-xương-khớp, xem thêm danh sách Doctors chuyên khoa CƠ XƯƠNG KHỚP.");
                let title = "Chuyên khoa khám bệnh";
                let subtitle = 'Thông tin Doctors chuyên khoa cơ-xương-khớp';
        ...
        try {
            let response1 = {
                "text": "Quy trình khám bệnh với Doctor Care gồm những bước sau:" +
                    "\n\n1. Bệnh nhân lựa chọn Doctors khám bệnh từ website." +
...
let sendMessageReplyDoctors = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                "text": "Doctors Care tự hào mang đến cho bạn đội ngũ Doctors hàng đầu, chất lượng và uy tín." +
                    "\n\nMột số Doctors tiêu biểu trên Doctors Care:"
            };
...
let sendMessageAllDoctors = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                "text": "Danh sách tất cả Doctors bạn xem thêm ở link bên dưới nhé:" +
