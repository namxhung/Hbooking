                ...
                if (data.message.length > 0) {
                    data.message.forEach((schedule, index) => {
                        ...
                        if (index === data.message.length - 1 && schedule.isDisable === true) {
                            html += `<div>
                                  Bác sĩ không thể hẹn vào ngày này. Vui lòng chọn thời gian khác
                            </div>`
                        }
                        ...
                        moreInfo = `
                         <div class="d-flex flex-column">
                                    <div>
                                                <span class="d-block mt-2">Chọn <i class="fa fa-hand-o-up" aria-hidden="true"></i>  </span>
                                    </div>
                    ...
                    html = `
                            <div>
                                 Doctor "${data.doctor.name}" không thể hẹn vào <b>${value}</b>. Vui lòng chọn lịch khám tiếp theo.
                            </div>
                    `;
                    moreInfo = '';
                }
                ...
                if (data.message.length > 0) {
                    ...
                    html = `
                            <div class="no-schedule">
                                  ...
                                  "${data.doctor.name}" không thể hẹn vào <b>${value}</b>. Vui lòng chọn lịch khám tiếp theo.
    ...
    $.ajax({
        ...
        success: function(data) {
            if (typeof (data.patient) === 'string') {
                alert("Unfortunately, this appointment has enough patients booked, please Chọn a different time.");
