...
export const transMailBookingNew = {
    ...
    template: (data) => {
        return `<h3>Cám ơn bạn đã đặt lịch khám tới Doctors Care </h3>
        <h4>Thông tin về lịch khám:</h4>
        <div>Doctor's name: ${data.doctor} </div>
        <div>Time: ${data.time}</div>
        <div>Date: ${data.date}</div>
        <div>Status: <b> Pending - A new appointment is waiting for confirmation</b></div>
        <h4>ĐỒ ÁN 2system will automatically send email notification when confirmed appointment is complete. Thank you !</h4>`;
    },
};
...
export const transMailRemedy= {
    ...
    template: (data) => {
        ...
        <br>
        <div>For example: Full name:: Nam Hung, with the registered phone number: 0123456789 and born: 1910, the extracted password is: <b> harypham-012-10 </b> </div>
        <br>
